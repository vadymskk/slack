...
class Slack
{
	...
	/**
	 * @var SlackAccess    Slack authorization data
	 */
	private $access;
	...
	{
		if ( $access_data ) {
			$this->access = new SlackAccess( $access_data );
		}
	 ...
	 *
	 * @return bool|SlackAccess    An access object with the authentication data in place
	...
	public function send_notification($text, $attachments = array())
	{
		if ( ! $this->is_authenticated() ) {
			throw new SlackAPIException( 'Access token not specified' );
		}
		...
		if ( $response->body != 'ok' ) {
			throw new SlackAPIException( 'There was an error when posting to Slack' );
		}
	}
